...
if ($mimeType eq 'text/plain') {
    system("pucko-mail-extractPart $path $part >/tmp/pucko.txt");
    system("chromium /tmp/pucko.txt");
} elsif ($mimeType eq 'text/html') {
    system("pucko-mail-extractPart $path $part >/tmp/pucko.html");
    system("chromium /tmp/pucko.html");
} elsif ($mimeType eq 'application/pdf') {
    system("pucko-mail-extractPart $path $part >/tmp/pucko.pdf");
    system("mupdf /tmp/pucko.pdf");
} elsif ($mimeType eq 'image/png') {
    system("pucko-mail-extractPart $path $part >/tmp/pucko.png");
